...
| R-005 | 不得要求提供赠品、回扣或者与采购无关的其他商品、服务 | `gifts_or_unrelated_goods_in_scoring_forbidden` | `scoring` | 将无关商品或无关服务作为评分条件，存在不当附加交易和评审合规风险 | 评分因素合规性 / 无关商品服务禁入评分 | 已关闭 | R-005 | `data/examples/rules/R-005/positive-01.json` | `tests/test_v2_compare.py` | 当前固定真实文件不触发 | 保留总类规则与隐性办公设备/会议保障变体，显性赠送场景拆分至 R-008 |
| R-008 | 评分项中要求赠送非项目物资 | `gifts_or_unrelated_goods_in_scoring_forbidden` | `scoring` | 评分项中要求赠送非项目物资，存在明显不当加分和评审合规风险 | 评分因素合规性 / 赠送非项目物资评分 | 复验中 | R-008 | `data/examples/rules/R-008/positive-01.json` | `tests/test_rule_registry_r008.py` | `data/results/v2/20260403-r008-real-replay` | 从 R-005 总类规则拆出的显性子规则，当前已完成独立注册、样本、测试和真实文件回放闭环 |
