...
def test_build_review_view_from_final_output_prefers_single_source_titles() -> None:
    final_output = {
        "formal_risks": [
            {
                "title": "正式风险标题",
...
""".strip(),
    ...
    monkeypatch.setattr("app.web.v2_app.find_run_dir", lambda run_id: run_dir if run_id == "single-source-run" else None)

    app = create_app()
    with app.test_request_context():
        result = load_result_by_run_id("single-source-run")
