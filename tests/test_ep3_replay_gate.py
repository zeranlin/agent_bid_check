...
def test_ep3_replay_classifies_business_domain_boundaries_in_real_files() -> None:
    ...
    diesel_pairs = {(item.metadata.get("section_title", ""), item.business_domain) for item in diesel_layer.evidences}
    assert ("5.2 投标人资格要求", "qualification") in diesel_pairs
    assert ("付款方式", "commercial") in diesel_pairs
    assert ("1.规格及技术参数", "technical_standard") in diesel_pairs
    assert all("business_domain_trace" in item.metadata for item in diesel_layer.evidences)
    ...
    fuzhou_pairs = {(item.metadata.get("section_title", ""), item.business_domain) for item in fuzhou_layer.evidences}
    assert ("资格审查不合格项：无", "qualification") in fuzhou_pairs
    assert ("（二）样品要求", "sample") in fuzhou_pairs
    assert ("（2）履约验收时间：计划于何时验收/供应商提出验收申请之日起_______日内组织验收", "acceptance") in fuzhou_pairs
    assert ("9验收标准及要求：", "acceptance") in fuzhou_pairs
    assert all("business_domain_trace" in item.metadata for item in fuzhou_layer.evidences)
